	...
	if (SUCCEEDED(hRes))
	{
		...
		for (int i=0 ; i < MsgRows->cRows; i ++)
		{
				  ...
				  || (mSRow == NULL))
			{
				...
				if (MsgRows->aRow[i].lpProps[ePR_MESSAGE_FLAGS].ulPropTag == PR_MESSAGE_FLAGS)
				{
					ReadStatus = MsgRows->aRow[i].lpProps[ePR_MESSAGE_FLAGS].Value.l;					
				}
				

				if (MsgRows->aRow[i].lpProps[ePR_SENDER_NAME].ulPropTag == PR_SENDER_NAME &&
					((ReadStatus & MSGFLAG_READ  == MSGFLAG_READ) || ReadStatus == -1  ))
				{
