...
BOOL CMainFrm::OnInitDialog()
{
	...
	m_IE.Navigate("http://www.softworking.com/SpamDogLite.asp",NULL,NULL,NULL,NULL);
...
//#endif
	...
	if (GetPrivateProfileInt("SysSetup","ArrangeConfirm",0,IniPath) == 1)
	{
		Settings.ArrangeConfirm = true;
	}
	else
	{
		Settings.ArrangeConfirm = false;
	}
	...
	if (GetPrivateProfileInt("SysSetup","RBLMaintain",0,IniPath) == 1)
	{
		Settings.RBLMaintain = true;

		GetPrivateProfileString("SysSetup","HeaderKeyStr","",Settings.HeaderKeyStr,sizeof(Settings.HeaderKeyStr),IniPath);
		Settings.HeaderKeyPos = GetPrivateProfileInt("SysSetup","HeaderKeyPos",0,IniPath);

	}
	else
	{
		Settings.RBLMaintain = false;
	}
	...
	if (GetPrivateProfileInt("SysSetup","AutoArrange",0,IniPath) == 1)
	{
	...
	Settings.ArrangeMin = GetPrivateProfileInt("SysSetup","ArrangeMin",30,IniPath) ;


	char CharsetStr[255]={0};
	GetPrivateProfileString("SysSetup","CharsetIg","",CharsetStr,sizeof(CharsetStr),IniPath);
	//以逗點格開
	char tempstr2[50]={0};
	int len = strlen(CharsetStr);
	int SavePos = 0;
	for (int i = 0 ; i < len ; i ++)
	{
		if (CharsetStr[i] == ',' || i == len - 1)
		{
		    if (i == len -1) i ++;
			strncpy(tempstr2,CharsetStr+SavePos, i - SavePos );		   
			CharsetIgList.insert(CharsetIgList.end(),tempstr2);
...
void CMainFrm::OnTimer(UINT nIDEvent) 
{
  ...
  CTime tnow((time_t) CTime::GetCurrentTime().GetTime());
  if (tnow.GetHour() == 6 &&  tnow.GetMinute() == 0) //06:00
  {

	    ::ShowWindow( m_IE.GetSafeHwnd(), SW_HIDE ); 
		bRefresh = true;
		COleVariant vFlags(long(REFRESH_COMPLETELY  ), VT_I4);
		m_IE.Refresh2(&vFlags);
  ...
  if (Settings.AutoArrange)
  {
	  ...
	  if ( ((dwNow-li.dwTime)/1000 ) >= 60 * Settings.ArrangeMin) //半個小時的 IDLE
	  {
...
void CMainFrm::OnButton5() 
{
	...
	if (!Settings.ArrangeConfirm)
	{
	  if (MessageBox("提醒您!整理信件會依寄件者名稱移動所有收信匣中的信件\r\n請確認是否執行?","SpamDog Lite",MB_OKCANCEL) == IDCANCEL)
	  {
	 	return;
	  }
	  else
	  {
			 ...
			 char IniPath[_MAX_PATH];
			   ...
			   WritePrivateProfileString("SysSetup","ArrangeConfirm","1",IniPath);
...
BEGIN_EVENTSINK_MAP(CMainFrm, CDialog)
    //{{AFX_EVENTSINK_MAP(CMainFrm)
	ON_EVENT(CMainFrm, IDC_EXPLORER1, 259 /* DocumentComplete */, OnDocumentCompleteExplorer1, VTS_DISPATCH VTS_PVARIANT)
	ON_EVENT(CMainFrm, IDC_EXPLORER1, 250 /* BeforeNavigate2 */, OnBeforeNavigate2Explorer1, VTS_DISPATCH VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT VTS_PBOOL)
	ON_EVENT(CMainFrm, IDC_EXPLORER1, 104 /* DownloadComplete */, OnDownloadCompleteExplorer1, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

void CMainFrm::OnDocumentCompleteExplorer1(LPDISPATCH pDisp, VARIANT FAR* URL) 
{
	...
	try
	{
		...
		if (pDocDisp)
		{
			...
			//pHtmlStyle->put_borderStyle(::SysAllocString(L"groove"));		
			pHtmlStyle->put_borderStyle(::SysAllocString(L"none"));		
		...
		::ShowWindow( m_IE.GetSafeHwnd(), SW_SHOW ); 
...
void CMainFrm::OnBeforeNavigate2Explorer1(LPDISPATCH pDisp, VARIANT FAR* URL, VARIANT FAR* Flags, VARIANT FAR* TargetFrameName, VARIANT FAR* PostData, VARIANT FAR* Headers, BOOL FAR* Cancel) 
{
	// TODO: Add your control notification handler code here
	::ShowWindow( m_IE.GetSafeHwnd(), SW_HIDE ); 
}
...
void CMainFrm::OnDownloadCompleteExplorer1() 
{
	// TODO: Add your control notification handler code here
	if (bRefresh)
	{
		  ...
		  bRefresh = false;
		  OnDocumentCompleteExplorer1(SavepDisp,NULL);		
	
	}
}

void CMainFrm::OnMenuitem32782() 
{
	// TODO: Add your command handler code here
		LPSRowSet pRows ;
		...
		HRESULT hRes = g_MapiEx->lpInboxFolder->GetHierarchyTable(0,&lpContentsTable);
		...
		for (int i = 0; i < pRows ->cRows; i++)
		{
			...
			{
			    if(strncmp(pRows->aRow[i].lpProps[ePR_DISPLAY_NAME].Value.lpszA,"[",1) != 0  )
				{
					...
					if (SUCCEEDED(hRes))
					{
						//ListOLFolder(hItem,pFolder); 只做第一層
						pFolder->EmptyFolder(NULL,NULL,DEL_ASSOCIATED);
						pFolder->Release();
					}
			   }
