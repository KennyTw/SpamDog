...
void CSetupFrm::OnButton1() 
{
	...
	//自動整理信件時間 , 要再 EndDialog 之前
	if (otherfrm->m_AutoArrange.GetCheck() == BST_CHECKED)
	{
		CString tmpstr;
		tmpstr="1";

		WritePrivateProfileString("SysSetup","AutoArrange",tmpstr,IniPath);
	}
	else
	{
	...
	CString TempCStr5;
	otherfrm->m_ArrangeMin.GetWindowText(TempCStr5);
	bool AutoArrangeNum = true;
	int  ArrangeMin = 0;
	if (TempCStr5.GetLength() > 0)
	{
	
		for (j=0 ; j < TempCStr5.GetLength() ; j++)
		{
			if (!isdigit(TempCStr5[j]))
			{
				AutoArrangeNum = false;
				break;
			}
		...
		if (AutoArrangeNum)
		{
			...
			ArrangeMin = atoi(TempCStr5.GetBuffer(TempCStr5.GetLength()));
		}

		if (AutoArrangeNum  && ArrangeMin > 0 )
		WritePrivateProfileString("SysSetup","ArrangeMin",TempCStr5,IniPath);
	}


	if (!AutoArrangeNum) 
	{
			AfxMessageBox("自動整理信件時間 , 需輸入數字及整數");
			return;
	}
